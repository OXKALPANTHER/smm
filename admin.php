    ...
    try {
        switch ($action) {
            case 'add_balance':
                ...
                $aabs = abs($amt);
                $stmt->bind_param("idss", $uid, $aabs, $type, $desc);
                $stmt->execute();
                $conn->commit();
            ...
            case 'delete_user':
                $uid = (int) ($in['user_id'] ?? 0);
                if ($uid === (int) $admin_id)
                    $reply(false, 'Huwezi kufuta akaunti yako mwenyewe.');
            ...
            case 'set_refill_status':
                ...
                if ($oid <= 0)
                    $reply(false, 'Order haijatambulika.');
                ...
                $stmt->bind_param("sii", $rs, $cleared, $oid);
                $stmt->execute();
...
foreach ($conn->query("SELECT status, COUNT(*) c FROM orders GROUP BY status")->fetch_all(MYSQLI_ASSOC) as $r) {
    ...
    if (strpos($s, 'complet') !== false)
        $statusCounts['completed'] += (int) $r['c'];
    elseif (strpos($s, 'pend') !== false || strpos($s, 'process') !== false)
        $statusCounts['pending'] += (int) $r['c'];
    elseif (strpos($s, 'cancel') !== false || strpos($s, 'fail') !== false)
        $statusCounts['failed'] += (int) $r['c'];
}
...
function abadge($status)
{
    ...
    if (strpos($s, 'pend') !== false || strpos($s, 'process') !== false)
...
<style>
    ...
    .sidebar {
        width: 250px;
        background: linear-gradient(180deg, #1b1f3b, #141830);
        color: #fff;
        position: fixed;
        height: 100vh;
        padding: 22px 18px;
    ...
    .mobile-top {
        ...
        padding: 14px 18px;
        border-radius: 14px;
        box-shadow: 0 4px 18px rgba(43, 54, 116, .06);
        margin-bottom: 18px;
        align-items: center;
    ...
    .stat-card {
        ...
        border-radius: 18px;
        padding: 1.2rem;
        box-shadow: 0 8px 26px rgba(43, 54, 116, .05);
        height: 100%;
    }
...
<div class="sidebar" id="sidebar">
    ...
    <a href="#refills" class="nav-link"><i class="bi bi-arrow-repeat"></i>
        Refills<?= count($refills) ? ' <span class="badge bg-danger rounded-pill ms-1">' . count($refills) . '</span>' : '' ?></a>
    <a href="notifications.php" class="nav-link"><i class="bi bi-bell"></i>
        Notifications<?= $unreadNotifications ? ' <span class="badge bg-danger rounded-pill ms-1">' . number_format($unreadNotifications) . '</span>' : '' ?></a>
...
<div class="main">
    <div class="mobile-top">
        ...
        <button class="btn btn-light" onclick="document.getElementById('sidebar').classList.toggle('show')"><i
    ...
    <div class="row g-3 mb-4">
        <div class="col-xl-3 col-md-6">
            <div class="stat-card d-flex justify-content-between align-items-center">
                ...
                <div class="ico" style="background:#eef0ff;color:var(--primary)"><i class="bi bi-people"></i></div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="stat-card d-flex justify-content-between align-items-center">
                ...
                <div class="ico" style="background:#e6fbfa;color:#00a8a3"><i class="bi bi-bag-check"></i></div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="stat-card d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <div class="val"><?= number_format($sales) ?> <small class="text-muted"
                            style="font-size:.9rem;">TZS</small></div>
                </div>
                <div class="ico" style="background:#e4faf3;color:#00876a"><i class="bi bi-graph-up-arrow"></i></div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="stat-card d-flex justify-content-between align-items-center">
                    ...
                    <div class="val" style="font-size:1rem;line-height:1.4;">
                        <?php if (!empty($providerBalances)): ?>
                            <?php foreach ($providerBalances as $providerName => $balance): ?>
                                <div><?= htmlspecialchars(strtoupper($providerName)) ?>: <?= number_format($balance) ?> <small
                ...
                <div class="ico" style="background:#fdeee9;color:#c0392b"><i class="bi bi-cloud-check"></i></div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-5">
            <div class="panel">
                ...
                <div style="height:230px;"><canvas id="statusChart"></canvas></div>
            </div>
        </div>
        <div class="col-lg-7">
            <div class="panel">
                <h6 class="fw-bold mb-3">Orders (Siku 7 zilizopita)</h6>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="stat-card">
                <div class="lbl">User Liability (jumla ya salio)</div>
                <div class="val text-primary"><?= number_format($sumBalance) ?> <small class="text-muted"
                        style="font-size:.9rem;">TZS</small></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div class="val text-success"><?= number_format($statusCounts['completed']) ?></div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card">
                <div class="lbl">Pending Orders</div>
                <div class="val" style="color:#b8860b"><?= number_format($statusCounts['pending']) ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="panel mb-4" id="refills">
        <h5 class="fw-bold mb-3"><i class="bi bi-arrow-repeat text-primary"></i> Refill Requests
            (<?= count($refills) ?>)</h5>
                    ...
                    <tbody>
                        <?php foreach ($refills as $r): ?>
                            <tr>
                                ...
                                <td class="small text-truncate" style="max-width:160px;"><a
                                        href="<?= htmlspecialchars($r['link']) ?>"
                                        target="_blank"><?= htmlspecialchars(mb_substr($r['link'], 0, 30)) ?></a></td>
                                <td><span
                                        class="badge-soft badge-warning"><?= htmlspecialchars($r['refill_status'] ?: 'requested') ?></span>
                                </td>
                                <td class="text-nowrap">
                                    <button class="btn btn-sm"
                                        style="background:#e8efff;color:#2f54eb;border:none;border-radius:10px;font-size:.72rem;"
                                        onclick="setRefill(<?= (int) $r['id'] ?>,'processing')">Processing</button>
                                    <button class="btn btn-sm"
                                        style="background:#e4faf3;color:#00876a;border:none;border-radius:10px;font-size:.72rem;"
                                        onclick="setRefill(<?= (int) $r['id'] ?>,'done')">Done</button>
                                    <button class="btn btn-sm"
                                        style="background:#fdeee9;color:#c0392b;border:none;border-radius:10px;font-size:.72rem;"
                                        onclick="setRefill(<?= (int) $r['id'] ?>,'rejected')">Reject</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <tbody>
                    <?php foreach ($users as $u): ?>
                        <tr>
                            ...
                            <td><span
                                    class="badge-soft badge-secondary text-uppercase"><?= htmlspecialchars($u['role']) ?></span>
                            </td>
                            <td><span
                                    class="badge-soft <?= ubadge($u['status']) ?>"><?= htmlspecialchars($u['status']) ?></span>
                            </td>
                            <td class="text-nowrap">
                                <button class="btn-ico" title="Salio"
                                    onclick='openBalance(<?= (int) $u['id'] ?>,"<?= htmlspecialchars(addslashes($u['username'])) ?>")'><i
                                        class="bi bi-cash-coin"></i></button>
                                <button class="btn-ico" title="Hariri"
                                    onclick='openManage(<?= htmlspecialchars(json_encode($u), ENT_QUOTES) ?>)'><i
                                        class="bi bi-gear"></i></button>
                                <button class="btn-ico" title="Futa"
                                    onclick='delUser(<?= (int) $u['id'] ?>,"<?= htmlspecialchars(addslashes($u['username'])) ?>")'><i
                ...
                <tbody>
                    <?php foreach ($orders as $o): ?>
                        <tr>
                            ...
                            <td><span
                                    class="badge-soft <?= abadge($o['status']) ?>"><?= htmlspecialchars($o['status']) ?></span>
                            </td>
                            ...
                            <td><button class="btn-ico" title="Badili status"
                                    onclick='openOrder(<?= (int) $o['id'] ?>,"<?= htmlspecialchars(addslashes($o['status'])) ?>")'><i
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Rekebisha Salio</h5><button class="btn-close"
            ...
            <div class="modal-footer border-0"><button class="btn-grad" style="width:auto;padding:.6rem 1.4rem;"
                    onclick="saveBalance()"><i class="bi bi-check2"></i> Hifadhi</button></div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="manageModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Simamia Mtumiaji</h5><button class="btn-close"
                    data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p class="text-muted small mb-3">User: <strong id="mgUser"></strong></p>
                <label class="form-label">Role</label>
                <select class="form-select mb-3" id="mgRole">
                    <option value="user">user</option>
                    <option value="moderator">moderator</option>
                    <option value="admin">admin</option>
                </select>
                <label class="form-label">Status</label>
                <select class="form-select" id="mgStatus">
            ...
            <div class="modal-footer border-0"><button class="btn-grad" style="width:auto;padding:.6rem 1.4rem;"
                    onclick="saveManage()"><i class="bi bi-check2"></i> Hifadhi</button></div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="orderModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Badili Order Status</h5><button class="btn-close"
                    data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer border-0"><button class="btn-grad" style="width:auto;padding:.6rem 1.4rem;"
                    onclick="saveOrder()"><i class="bi bi-check2"></i> Hifadhi</button></div>
        </div>
    </div>
</div>

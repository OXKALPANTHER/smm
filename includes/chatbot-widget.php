...
(function(){
  ...
  async function ask(text){text=text.trim();if(!text||send.disabled)return;addMessage(text,'user');history.push({role:'user',content:text});input.value='';send.disabled=true;const loading=addSystem('Thinking...');try{const response=await fetch('chatbot.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:history})});const data=await response.json();loading.remove();if(!response.ok||!data.success){addMessage(data.message||'Live support is temporarily unavailable.','bot');return;}addMessage(data.message,'bot');history.push({role:'assistant',content:data.message});}catch(error){loading.remove();addMessage('I could not connect to live support. Please try again or use WhatsApp support.','bot');}finally{send.disabled=false;input.focus();}}
